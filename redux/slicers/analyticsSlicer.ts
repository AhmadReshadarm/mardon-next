...
>(
  ...
  async function (
    payload: AnalyticsStaticPayload,
    { rejectWithValue },
  ): Promise<any> {
    try {
...
>(
  ...
  async function (
    payload: AnalyticsDynamicPayload,
    { rejectWithValue },
  ): Promise<any> {
    try {
  ...
  async function (payload: UsersPayload, { rejectWithValue }): Promise<any> {
    try {
      const resp = await axiosInstance.get(
        `/users?createdFrom=${payload.createdFrom}&createdTo=${payload.createdTo}T23:59:59.999Z&offset=${payload.offset}&limit=${payload.limit}`,
      );
      return resp.data;
    } catch (error: any) {
      return rejectWithValue(getErrorMassage(error.response.status));
...
const analyticsSlicer = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchDynamicAnalytics.fulfilled, (state, action) => {
      ...
      .addCase(fetchAnalyticsUsers.fulfilled, (state, action) => {
